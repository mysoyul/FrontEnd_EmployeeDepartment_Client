...
* 저는 tailwincss 를 처음 사용합니다. tailwincss에 대한 튜토리얼을 markdown 형식으로 작성해 주세요. tailwindcss 적용전의 css/style_old.css 와 새로 작성된 src/style.css도 비교해 주세요. 수정된 index.html에서도 변경된 사항을 포함하여 비교하는 내용을 튜토리얼.md에 포함시켜주세요.

Vite 프로젝트에 환경설정 파일 .env.development 와 .env.production 생성해서 baseUrl 정보를 포함해서 환경별로 다른 config 파일을 생성하고 VITE_MIGRATION.md 를 갱신해 줘